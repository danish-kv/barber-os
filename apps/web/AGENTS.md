...
- Commands (from repo root): `pnpm dev:web | lint | typecheck | test | build`. Web `test` runs `scripts/flow-test.mts` + the mandatory `scripts/storyline-test.mts`.
- Availability engine, checkout math and queue wait estimation live in `@barbershop-os/domain`; `lib/availability.ts` and parts of `lib/store.ts`/`lib/selectors.ts`/`pos.tsx` are thin adapters binding demo data to it. Change business rules in the package, not the adapters.
